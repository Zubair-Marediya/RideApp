...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** REPLACE THIS LINE ***"
    restaurant_to_centroid = []
    for current_restaurant in restaurants:
        restaurant_to_centroid.append([find_closest(restaurant_location(current_restaurant), centroids), current_restaurant])
...
def find_centroid(cluster):
    ...
    cluster_latitude = []
    cluster_longitude = []
    for current_location in cluster:
        cluster_latitude.append(restaurant_location(current_location)[0])
        cluster_longitude.append(restaurant_location(current_location)[1])
    return [mean(cluster_latitude), mean(cluster_longitude)]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        "*** REPLACE THIS LINE ***"
        centroids = []
        for current_cluster in group_by_centroid(restaurants, old_centroids):
            centroids.append(find_centroid(current_cluster))
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    for current_restaurant in restaurants:
        rating = predictor(current_restaurant)
        if current_restaurant in reviewed:
                rating = user_rating(user, restaurant_name(current_restaurant))
        ratings.update( {restaurant_name(current_restaurant): rating} )
    return ratings
    # END Question 9
...
def search(query, restaurants):
    ...
    search_list = []
    for current_restaurant in restaurants:
        for current_category in restaurant_categories(current_restaurant):
            if current_category == query:
                search_list.append(current_restaurant)
    return search_list
    # END Question 10
